...
describe("connected tiles", () => {
    ...
    it("can handle multiple horizontally connected tiles", () => {
        expect(connectedTiles([[TileType.NORTH_WEST, TileType.START, TileType.EAST_WEST]], {x: 1, y: 0})).to.deep.equals([{x: 2, y: 0}, {x: 0, y: 0}]);
    })
    it("can handle multiple vertically connected tiles", () => {
        expect(connectedTiles([[TileType.NORTH_SOUTH], [TileType.START], [TileType.NORTH_WEST]], {x: 0, y: 1})).to.deep.equals([{x: 0, y: 0}, {x: 0, y: 2}]);
    })
})